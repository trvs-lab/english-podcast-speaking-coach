...
**Architecture:** 只调整两个现有职责边界：`SKILL.md` 用简短的正向行为合同控制课次范围、学习块连贯性、整合降阶和证据措辞；`references/evaluation.md` 把 EnglishPod 第 25 课暴露的问题固定成可重复的轨迹回归。完整课文仍保留 8–15 个候选表达，3–5 个只表示单个学习块的容量，剩余内容通过后续学习块继续。
...
- 普通开放请求形成一节负担可控的课；明确要求尽可能掌握整篇时，才自动跨多个学习块继续。
...
- 普通“学这一课”请求默认完成一个学习块；第一块独立表现稳定且整合顺利时，可以继续第二块，第二块后收束并保存剩余候选。
- “尽可能掌握整篇课文”请求在没有负担信号时继续下一块，但每 3–5 个主要新表达后必须出现检查点。
...
- 课末小结为主动表达提供中文用途和具体回忆提示，并区分继续练习、剩余内容和场景词汇。
...
- Produces: agent 可直接执行的课次边界与块级任务主线。
...
- [ ] **Step 2: 用表现证据控制普通请求的课次长度**

  把普通开放请求的自动续块规则替换为：

  ```markdown
  学习者明确希望尽可能掌握整篇课文时，没有出现学习负担信号就自动进入下一学习块。

  学习者只提出“学这一课”等开放请求时，默认完成一个学习块。第一块多数核心表达已经形成无提示新情境证据，且块末整合可以独立完成时，可以继续第二块；第二块后进入小结并保存剩余候选。
  ```

  该规则让 agent 根据学习证据决定是否进入第二块，不向学习者展示课程模式选择。
...
- [ ] **Step 4: 运行两个课次边界微测试**
  ...
  **场景 B**
  ...
  ```text
  学习者说：“我希望尽可能掌握整篇课文。”
  课文可提取 15 个高价值表达，前两块回答稳定，没有负担信号。请说明第二块结束后的下一步。
  ```
...
- [ ] **Step 4: 把课末小结改成可直接复习的固定形状**
  ...
  保留“还要再练一下”“这节课碰到的词”“下次从这里继续”三个现有槽位。
...
- [ ] **Step 1: 用相同请求生成 fresh-context 课堂轨迹**
  ...
  用基线中的候选清单核对完整读取结果：`come up with a clear plan`、`backup/contingency plan`、`figure out`、`have an impact on`、`leave ... to someone`、`head up`、`forecast`、`critical`、`flu outbreak`、`avoid getting infected`、`It’s come to my attention that`、`outline specific actions`、`draft`、`move on to`、`maintain essential services`。测试不要求把这些候选全部塞入同一节课。
...
- [ ] **Step 4: 最终人工审阅**
  ...
  - agent 是否可以自行决定第一块和第二块，而不把判断交给学习者？
